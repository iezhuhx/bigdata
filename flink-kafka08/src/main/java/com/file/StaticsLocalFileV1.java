...
public class StaticsLocalFileV1 {
    public static void main(String[] args) {
        ...
        //加载数据源到 DataSet
        DataSet<String> text =env.readTextFile("d:/data/bigdata/test.txt") ;//env.readTextFile("d:/data/bigdata/test.txt");
                ...
                text.flatMap(new FlatMapFunction<String, WordWithCount>() {
                    ...
                    public void flatMap(String s, Collector<WordWithCount> collector) throws Exception {
                        ...
                        for (String token : tokens) {
                            if (token.length() > 0) {
                                collector.collect(new WordWithCount(token.replace(" ",""), 1));
                            }
                        }
                    }
                })
